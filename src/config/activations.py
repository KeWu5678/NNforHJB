...
The ``ACTIVATIONS`` table maps a name to ``(callable, use_sphere)``. ``use_sphere``
records whether the activation is positively homogeneous (safe to sample candidate
directions on the sphere S^d) — it travels *with* the function so a config cannot
pair an activation with the wrong geometry.

Config stores the activation as a **string** (OmegaConf holds only primitives);
:func:`get_activation` resolves it to an :class:`ActivationSpec` at build time.
...
class ActivationSpec:
    """A resolved activation: its registry name, callable, and sphere geometry."""
