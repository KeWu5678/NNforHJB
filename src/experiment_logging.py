...
class RunRecordWriter:
    """Write run records from a runner's flat result summary."""
    ...
    def run_id(self, summary: dict[str, Any]) -> str:
        ...
        for field_name in self.id_fields:
            ...
            parts.append(f"seed{value}" if field_name == "seed" else str(value))
        return "_".join(parts)

    def config(self, summary: dict[str, Any]) -> dict[str, Any]:
        return {field_name: summary[field_name] for field_name in self.config_fields if field_name in summary}
