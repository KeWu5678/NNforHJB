...
from model_outerweights import network_outerweights
...
if __name__ == "__main__":
    ...
    # Data inspection
    logger.info(f"Loaded dataset from {path}, shape: {dataset.shape}, dtype: {dataset.dtype}")

    # Initialize the model with zero neurons (no hidden layer)
    init_weights = None
    init_bias = None
    vdp_model = model(torch.relu, 2.0, regularization, optimizer='SSN', loss_weights=(1.0, 0.0))
    ...
    weights_history = {
        ...
        'weights': [weight.copy()],
        'biases': [bias.copy()],
        'train_loss': [model_result.losshistory.loss_train],
        'test_loss': [model_result.losshistory.loss_test],
        'test_metrics': [],
    ...
    for i in range(num_iterations - 1):   
        weight_temp, bias_temp = insertion(data, model, M, alpha)
        weight_temp = weight_temp.numpy(); bias_temp = bias_temp.numpy()
