...
data_dir = os.path.dirname(os.path.abspath(__file__))  # Current directory path

# Load and combine datasets
combined_data = np.load(os.path.join(data_dir, 'VDP_beta_0.1_grid_30x30.npy'))
print(f"Loaded original dataset with {len(combined_data)} points")

# Try to load the odd-indexed data
try:
    odd_indexed_data = np.load(os.path.join(data_dir, 'VDP_beta_0.1_grid_30x30_odd_indices.npy'))
    print(f"Loaded odd-indexed dataset with {len(odd_indexed_data)} points")
    
    # Combine with original data
    dtype = combined_data.dtype
    all_combined_data = np.zeros(len(combined_data) + len(odd_indexed_data), dtype=dtype)
    all_combined_data[:len(combined_data)] = combined_data
    all_combined_data[len(combined_data):] = odd_indexed_data
    
    print(f"Combined both datasets, total points: {len(all_combined_data)}")
    combined_data = all_combined_data
except FileNotFoundError:
    print("Odd-indexed dataset not found. Using only original data.")
...
combined_output_file = os.path.join(data_dir, 'VDP_beta_0.1_grid_combined.npy')
np.save(combined_output_file, valid_data)
print(f"Saved combined dataset ({len(valid_data)} points) to {combined_output_file}")
...
ax.set_title('Combined VDP Data (β=0.1): Original + Odd-Indexed Grids')
...
plt.savefig(os.path.join(data_dir, 'VDP_3D_value_function_combined_grids.png'), dpi=300)

# Save the figure as a pickle for interactive use later
pickle_file = os.path.join(data_dir, 'VDP_3D_value_function_combined_grids.pickle')
...
plt.xlabel('x₀[0]')
plt.ylabel('x₀[1]')
plt.title('Value Function with Gradient Vectors (Combined Original and Odd-Indexed Data)')
plt.grid(True, alpha=0.3)

# Save as high-resolution image
plt.savefig(os.path.join(data_dir, 'VDP_gradient_vectors_combined_data.png'), dpi=300, bbox_inches='tight')
plt.show()

ini = np.load(os.path.join(data_dir, "VDP_beta_0.1_failed_ini_30x30.npy"))
print(f"Loaded {len(ini)} failed initial conditions from original dataset")

# Try to load failed initial conditions from odd-indexed data
try:
    odd_ini = np.load(os.path.join(data_dir, "VDP_beta_0.1_failed_ini_30x30_odd_indices.npy"))
    print(f"Loaded {len(odd_ini)} failed initial conditions from odd-indexed dataset")
    
    # Combine failed initial conditions
    all_failed_ini = np.vstack((ini, odd_ini))
    print(f"Combined failed initial conditions: {len(all_failed_ini)} points")
    ini = all_failed_ini
    
    # Save combined failed initial conditions
    failed_combined_file = os.path.join(data_dir, "VDP_beta_0.1_failed_ini_combined.npy")
    np.save(failed_combined_file, ini)
    print(f"Saved combined failed initial conditions ({len(ini)} points) to {failed_combined_file}")
except FileNotFoundError:
    print("Failed odd-indexed initial conditions file not found.")
...
plt.xlabel('x₀[0]')
plt.ylabel('x₀[1]')
plt.title('Failed Initial Conditions Compared to Valid Data (Original + Odd-Indexed)')
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig(os.path.join(data_dir, 'VDP_failed_initial_conditions_combined.png'), dpi=300, bbox_inches='tight')
plt.show()
