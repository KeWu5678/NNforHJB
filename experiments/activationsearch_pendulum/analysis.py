#!/usr/bin/env python3
"""Analyse the activationsearch_pendulum experiment.

Reads the per-run JSON records written by ``scripts/train.py`` under the Hydra
multirun directory (``multirun/activationsearch_pendulum/<n>/``) and produces a
Markdown results table and the sparsity/accuracy tradeoff figure under
``experiments/activationsearch_pendulum/``. Selection rule: the best
(lowest-score) gamma per (activation, loss, seed). Reproduce with
...
EXPERIMENT = "activationsearch_pendulum"
...
def load_rows() -> list[dict[str, Any]]:
    ...
    if not records:
        raise FileNotFoundError(
            f"no run records under {MULTIRUN_DIR} — run `make activationsearch_pendulum` first"
        )
    rows = []
    for path in records:
...
def main() -> int:
    ...
    plot_score_tradeoff(
        ...
        title="activationsearch_pendulum: sparsity/accuracy tradeoff",
