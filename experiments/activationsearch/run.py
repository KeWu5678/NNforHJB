#!/usr/bin/env python3
"""Run the activationsearch experiment."""
...
def gamma_label(gamma: float) -> str:
    return f"{float(gamma):g}".replace(".", "p").replace("-", "m")


def run_id(*, activation: str, loss: str, gamma: float, seed: int) -> str:
    return f"{activation}_{loss}_gamma{gamma_label(gamma)}_seed{seed}"


def make_pdap_cfg(raw: dict[str, Any], *, activation: str, loss: str, gamma: float, seed: int):
    model = raw["model"]
...
def run_point(experiment_name: str, raw: dict[str, Any], data: dict[str, np.ndarray],
              ...
              *, activation: str, loss: str, gamma: float, seed: int,
              record_dir: Path, artifact_dir: Path) -> dict[str, Any]:
    set_seed(seed)
    cfg = make_pdap_cfg(raw, activation=activation, loss=loss, gamma=gamma, seed=seed)
    pdap = PDAP.from_config(cfg, data)
    started = time.time()
    result = pdap.fit_from_config(cfg.training, verbose=False)
    elapsed_s = round(time.time() - started, 3)

    rid = run_id(activation=activation, loss=loss, gamma=gamma, seed=seed)
...
def write_outputs(rows: list[dict[str, Any]]) -> None:
    ...
    for key, group in itertools.groupby(
        sorted(rows, key=lambda r: (r["activation"], r["loss"], r["seed"], r["score"])),
        key=lambda r: (r["activation"], r["loss"], r["seed"]),
    ):
...
def main() -> int:
    ...
    rows = []
    points = itertools.product(raw["seeds"], raw["activations"], raw["losses"], raw["gammas"])
    for index, (seed, activation, loss, gamma) in enumerate(points):
        if args.limit is not None and index >= args.limit:
            break
        get_activation(activation)  # validate early
        row = run_point(
            args.output_name, raw, data, normalizer_info,
            activation=activation, loss=loss, gamma=gamma, seed=seed,
