#!/usr/bin/env python3
"""Analyse the activationsearch_VDP experiment.

Reads the per-run JSON records written by ``scripts/train.py`` under the Hydra
multirun directory (``multirun/activationsearch_VDP/<n>/``) and produces a
Markdown results table and the sparsity/accuracy tradeoff figure under
``experiments/activationsearch_VDP/``. Selection rule: the best (lowest-score)
gamma per (activation, loss, seed). Reproduce with ``make activationsearch_VDP``.
"""
...
EXPERIMENT = "activationsearch_VDP"
...
def load_rows() -> list[dict[str, Any]]:
    ...
    if not records:
        raise FileNotFoundError(
            f"no run records under {MULTIRUN_DIR} — run `make activationsearch_VDP` first"
        )
    rows = []
    for path in records:
        ...
        rows.append({
            "kind": model["kind"], "insertion": model["insertion"],
...
def best_per_cell(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def cell(row: dict[str, Any]) -> tuple:
        return (row["kind"], row["insertion"], row["activation"], row["loss"], row["seed"])
...
def main() -> int:
    ...
    table = format_table(
        best,
        ["kind", "insertion", "activation", "loss", "seed", "gamma", "neurons", "val_h1", "score"],
        headers={"val_h1": "Val H1"},
        formats={"gamma": "{:g}", "val_h1": "{:.2e}", "score": "{:.2e}"},
        title="Best gamma per kind/insertion/activation/loss/seed",
    )
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    (OUTPUT_DIR / "results.md").write_text(
        f"# {EXPERIMENT} Results\n\n{table}\n", encoding="utf-8")

    # add a composite key so the plot legend distinguishes both variant and loss
    for row in best:
        row["variant"] = f"{row['kind']}/{row['insertion']} {row['loss']}"
    plot_score_tradeoff(
        best,
        x="neurons", y="val_h1", label="activation", color="variant",
        title="activationsearch_VDP: sparsity/accuracy tradeoff",
